  ...
  class SubscriptionsController < ApplicationController
    ...
    def create
      ...
      if @subscription.save
        flash[:notice] = t('koudoku.upgrade_success')
        redirect_to owner_subscription_path(@owner, @subscription)
      else
        flash[:error] = t('koudoku.transaction_problem')
        render :new
      end
    end
    ...
    def cancel
      flash[:notice] = t('koudoku.cancel_success')
    ...
    def update
      if @subscription.update_attributes(subscription_params)
        flash[:notice] = t('koudoku.update_success')
        redirect_to owner_subscription_path(@owner, @subscription)
      else
        flash[:error] = t('koudoku.transaction_problem')
        render :edit
    ...
    def subscription_params
      ...
      if defined?(ActionController::StrongParameters)
        params.require(:subscription).permit(:plan_id, :stripe_id, :current_price, :credit_card_token, :card_type, :last_four, :card_expiration, :error, :guid, :fee_amount)
      else
